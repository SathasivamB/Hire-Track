...
import { PlusCircle, Database, Check, Users, Target, Trash, Trash2, AlertTriangle, BarChart3, TrendingUp, Search, Edit, LogOut } from 'lucide-react';
...
const QuestionManager = ({ setIsAdminLoggedIn }) => {
  ...
  const handleBulkDelete = (companyId) => {
    if (!companyId) return;
    
    const confirmMsg = companyId === 'all' 
      ? 'Are you absolutely sure? This will delete ALL questions in the entire bank!' 
      : `Delete all questions belonging to ${companies.find(c => c.id === companyId)?.name}?`;
      
    if (window.confirm(confirmMsg)) {
      fetch(`/api/questions/bulk/${companyId}`, { method: 'DELETE' })
        .then(res => res.json())
        .then(data => {
          fetchData();
          setNotification(`Successfully deleted ${data.count} questions.`);
  ...
  const handleDeleteSelected = () => {
    if (window.confirm(`Delete ${selectedQuestions.length} selected questions?`)) {
      Promise.all(selectedQuestions.map(id => 
        fetch(`/api/questions/${id}`, { method: 'DELETE' })
      )).then(() => {
        fetchData();
        setNotification(`${selectedQuestions.length} questions deleted`);
        setSelectedQuestions([]);
        setTimeout(() => setNotification(null), 3000);
      });
    }
  ...
  const renderAnalytics = () => {
    ...
    const avgExp = users.reduce((acc, u) => acc + (u.totalExp || u.exp || 0), 0) / (totalUsers || 1);
      ...
      <div className="fade-up" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '20px' }}>
        ...
        <div className="dash-card" style={{ gridColumn: 'span 4' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {users.sort((a,b) => (b.totalExp || b.exp || 0) - (a.totalExp || a.exp || 0)).slice(0, 5).map((u, idx) => (
              <div key={u.email} style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '12px', background: '#f8fafc', borderRadius: '12px' }}>
                ...
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontWeight: 'bold', color: 'var(--brand-blue)' }}>{u.totalExp || u.exp || 0} EXP</div>
  ...
  return (
    <div className="container" style={{ paddingBottom: '60px' }}>
      ...
      <div className="dashboard-header-clean fade-up" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <h2 className="section-title">Placement Cell Dashboard v2.0</h2>
        ...
        <button 
          ...
          className="btn" 
          style={{ background: '#f1f5f9', color: '#64748b', display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 20px', borderRadius: '12px', fontWeight: '600', border: 'none', cursor: 'pointer' }}
        >
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                {['optionA', 'optionB', 'optionC', 'optionD'].map((opt, i) => (
                  <div key={opt} style={{ display: 'flex', alignItems: 'center', gap: '12px', background: '#f8fafc', padding: '4px 12px', borderRadius: '12px', border: '1px solid var(--border-light)' }}>
                    <span style={{ fontWeight: '800', color: 'var(--brand-blue)', fontSize: '1rem', minWidth: '25px' }}>{String.fromCharCode(65+i)})</span>
                    <input 
                      required 
                      placeholder={`Enter option ${String.fromCharCode(65+i)}...`} 
                      value={formData[opt]} 
                      onChange={(e) => setFormData({...formData, [opt]: e.target.value})} 
                      style={{ flex: 1, padding: '10px 0', border: 'none', background: 'transparent', outline: 'none', fontSize: '0.9rem' }} 
                    />
                  </div>
                ))}
              </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', gap: '20px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                ...
                {selectedQuestions.length > 0 && (
                  <button 
                    ...
                    className="btn" 
                    style={{ background: '#fee2e2', color: '#ef4444', border: '1px solid #fecaca', display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 16px', borderRadius: '10px' }}
                  >
                    <Trash2 size={16} /> Delete Selected ({selectedQuestions.length})
                  </button>
                )}
              </div>
              ...
              <div style={{ display: 'flex', gap: '12px' }}>
                <select 
                  ...
                  style={{ padding: '10px', borderRadius: '12px', border: '1px solid #fee2e2', color: '#ef4444', background: '#fff', fontSize: '0.875rem', cursor: 'pointer', outline: 'none' }}
                  defaultValue=""
                >
                  <option value="" disabled>Bulk Delete Questions</option>
                  <option value="all" style={{ fontWeight: 'bold' }}>Delete ALL Questions</option>
                  {companies.map(c => (
                    <option key={c.id} value={c.id}>Delete {c.name} Questions</option>
                  ))}
                </select>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                ...
                <tbody>
                  {filteredQuestions.slice().reverse().map(q => (
                    <tr key={q.id} style={{ borderBottom: '1px solid var(--border-light)', background: selectedQuestions.includes(q.id) ? '#f8fafc' : 'transparent' }}>
              ...
              <tbody>
                {users.map(u => (
                  <tr key={u.email} style={{ borderBottom: '1px solid var(--border-light)' }}>
                    ...
                    <td style={{ padding: '16px' }}><strong style={{ color: 'var(--brand-blue)' }}>{u.totalExp || u.exp || 0}</strong></td>
                    <td style={{ padding: '16px' }}><span style={{ background: '#f1f5f9', padding: '4px 10px', borderRadius: '20px', fontSize: '0.75rem', fontWeight: 'bold' }}>Lvl {Math.floor((u.totalExp || u.exp || 0) / 500) + 1}</span></td>
                    ...
                    <td style={{ padding: '16px', textAlign: 'center' }}><button onClick={() => handleDeleteUser(u.email)} style={{ background: '#fee2e2', color: '#ef4444', border: 'none', padding: '8px', borderRadius: '8px', cursor: 'pointer' }}><Trash size={16}/></button></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="fade-up">
           <div className="dash-card" style={{ marginBottom: '24px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px' }}>
                 <input className="input-field" placeholder="Company Name (e.g. Google)" value={companyForm.name} onChange={e => {
                   const newName = e.target.value;
                   if (editingCompany) {
                     setCompanyForm({...companyForm, name: newName});
                   } else {
                     setCompanyForm({...companyForm, name: newName, id: newName.replace(/\s+/g, '')});
                   }
                 }} />
                 <input className="input-field" placeholder="Logo Initial or Image URL (e.g. G or https://...)" value={companyForm.logo} onChange={e => setCompanyForm({...companyForm, logo: e.target.value})} />
