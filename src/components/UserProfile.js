      ...
      <div className="dash-card fade-up" style={{ padding: '40px' }}>
        {isEditing ? (
          <form onSubmit={handleUpdate} style={{ display: 'flex', flexDirection: 'column', gap: '20px', maxWidth: '600px' }}>
            <h3 style={{ color: 'var(--text-primary)' }}>Edit Professional Details</h3>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <label style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--text-primary)' }}>Display Name</label>
                <input type="text" className="input-field" value={newName} onChange={(e) => setNewName(e.target.value)} required style={{ background: 'var(--bg-primary)', color: 'var(--text-primary)', border: '1px solid var(--border-light)' }}/>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <label style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--text-primary)' }}>Roll No.</label>
                <input type="text" className="input-field" value={newStudentId} onChange={(e) => setNewStudentId(e.target.value)} required style={{ background: 'var(--bg-primary)', color: 'var(--text-primary)', border: '1px solid var(--border-light)' }}/>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <label style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--text-primary)' }}>Department</label>
                <select className="input-field" value={newDept} onChange={(e) => setNewDept(e.target.value)} style={{ background: 'var(--bg-primary)', color: 'var(--text-primary)', border: '1px solid var(--border-light)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '40px' }}>
            ...
            <div style={{ flex: 1 }}>
              <h2 style={{ fontSize: '2.5rem', fontWeight: '800', marginBottom: '8px', color: 'var(--text-primary)' }}>{userProfile.name}</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '24px', marginTop: '40px' }}>
          <div style={{ background: 'var(--card-bg)', backdropFilter: 'blur(10px)', padding: '24px', borderRadius: '20px', border: '1px solid var(--border-light)' }}>
            <h4 style={{ marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--text-primary)' }}><Shield size={18} color="var(--brand-blue)"/> Account Security</h4>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.9rem' }}>
                <span style={{ color: 'var(--text-tertiary)' }}>Login Method</span>
                <span style={{ fontWeight: '600', color: 'var(--text-primary)' }}>Institutional SSO</span>
              </div>
            </div>
          </div>
          
          <div style={{ background: 'var(--card-bg)', backdropFilter: 'blur(10px)', padding: '24px', borderRadius: '20px', border: '1px solid var(--border-light)' }}>
            <h4 style={{ marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--text-primary)' }}><Award size={18} color="#f59e0b"/> Rankings</h4>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.9rem' }}>
                <span style={{ color: 'var(--text-tertiary)' }}>College Rank</span>
                <span style={{ fontWeight: '900', color: 'var(--brand-blue)', fontSize: '1.1rem' }}>#{globalRank || '--'}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.9rem' }}>
                <span style={{ color: 'var(--text-tertiary)' }}>Current Level</span>
                <span style={{ fontWeight: '700', color: 'var(--text-primary)' }}>Lvl {level}</span>
              </div>
            </div>
          </div>
        </div>
