...
import { Medal, Crown, TrendingUp, Search, Zap } from 'lucide-react';
...
const Leaderboard = ({ userProfile }) => {
  ...
  useEffect(() => {
    fetch('/api/users')
      .then(res => res.json())
      .then(data => {
        // Sort by totalExp descending
        const sorted = data.sort((a, b) => (b.totalExp || b.exp || 0) - (a.totalExp || a.exp || 0));
        setUsers(sorted);
      })
      ...
      <div className="leaderboard-list fade-up delay-100">
        <div className="dash-card" style={{ padding: '0', overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
            <thead>
              <tr style={{ background: 'var(--bg-secondary)', borderBottom: '2px solid var(--border-light)', color: 'var(--text-tertiary)', fontSize: '0.85rem', fontWeight: 'bold' }}>
                  ...
                  <tr key={u.email || u.id} style={{ 
                    ...
                    <td style={{ padding: '16px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        ...
                        <div>
                          <div style={{ fontWeight: '700', color: 'var(--text-primary)' }}>{u.name} {isCurrentUser && <span style={{ color: 'var(--brand-blue)', fontSize: '0.7rem' }}>(You)</span>}</div>
                          <div style={{ fontSize: '0.75rem', color: 'var(--text-tertiary)' }}>{u.studentId}</div>
                        </div>
                      </div>
                    </td>
                    <td style={{ padding: '16px' }}>
                       <span style={{ background: 'var(--bg-secondary)', padding: '4px 10px', borderRadius: '8px', fontSize: '0.75rem', fontWeight: 'bold', color: 'var(--text-secondary)' }}>
                         {u.department || 'General'}
                       </span>
                    </td>
                    <td style={{ padding: '16px' }}>
                       <div style={{ fontWeight: '800', display: 'flex', alignItems: 'center', gap: '4px', color: 'var(--text-primary)' }}>
                         <Zap size={14} fill="#f59e0b" color="#f59e0b" /> {u.totalExp || u.exp || 0}
                       </div>
                    </td>
                    <td style={{ padding: '16px' }}>
                       <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                         <TrendingUp size={14} color="#10b981" />
                         <span style={{ fontWeight: '700', color: 'var(--text-primary)' }}>Lvl {Math.floor((u.totalExp || u.exp || 0) / 500) + 1}</span>
                       </div>
                    </td>
                  </tr>
                );
