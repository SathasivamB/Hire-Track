...
app.put('/api/requests/:id', (req, res) => {
  ...
  if (requestIndex !== -1) {
    ...
    const userIndex = db.users.findIndex(u => u.email === request.userId);
    if (userIndex !== -1) {
      if (status === 'approved') {
...
app.put('/api/users/:identifier', (req, res) => {
  ...
  const db = readDB();
  const index = db.users.findIndex(u => u.id.toString() === identifier || u.email === identifier);
  if (index !== -1) {
    // Revoke permission after update if it was 'granted'
    const newPermission = db.users[index].editPermission === 'granted' ? 'none' : db.users[index].editPermission;
    db.users[index] = { ...db.users[index], ...updatedData, editPermission: newPermission };
...
app.delete('/api/questions/bulk/:companyId', (req, res) => {
  ...
  if (companyId === 'all') {
    count = db.questions.length;
    db.questions = [];
    addLog('Question Bank Wiped', `Admin deleted all ${count} questions from the bank.`);
  } else {
    const originalCount = db.questions.length;
    db.questions = db.questions.filter(q => q.company !== companyId);
    count = originalCount - db.questions.length;
    addLog('Bulk Question Delete', `Admin deleted all ${count} questions for company: ${companyId}.`);
  }
  
  writeDB(db);
  res.json({ message: `Deleted ${count} questions`, count });
});
