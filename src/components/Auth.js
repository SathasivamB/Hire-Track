...
const Auth = ({ setIsLoggedIn, setUserProfile }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!email.toLowerCase().endsWith('@skasc.ac.in')) {
      setError('Access Denied: Only @skasc.ac.in email addresses are permitted.');
      return;
    }
    ...
    const payload = isLogin
      ...
      : { name, email, password: password.trim(), studentId, department };
    ...
    <div className="auth-container">
      <div className="auth-card fade-up">
        <div className="auth-header">
          ...
          <h2 style={{ color: 'var(--text-primary)' }}>HireTrack</h2>
          <p style={{ fontWeight: 'bold', color: 'var(--brand-blue)', fontSize: '0.9rem', marginTop: '-10px', marginBottom: '20px' }}>College Placement Portal</p>
          <p style={{ color: 'var(--text-secondary)' }}>{isLogin ? 'Welcome back, student! Login to continue.' : 'Register for your college placement journey.'}</p>
        </div>

        {error && (
          <div style={{ background: 'rgba(239, 68, 68, 0.1)', color: 'var(--error)', padding: '12px 16px', borderRadius: '10px', marginBottom: '16px', fontSize: '0.875rem', border: '1px solid rgba(239, 68, 68, 0.3)' }}>
            {error}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="auth-form">
            ...
            <>
              ...
              <div className="form-row" style={{ display: 'flex', gap: '12px' }}>
                ...
                <div className="form-group" style={{ flex: 1 }}>
                  ...
                  <select value={department} onChange={(e) => setDepartment(e.target.value)} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid var(--border-light)', background: 'var(--bg-primary)', color: 'var(--text-primary)' }}>
          ...
          <button type="submit" className="btn btn-brand w-full auth-btn" disabled={loading}>
            {loading ? 'Please wait...' : (isLogin ? <><LogIn size={18} /> Login</> : <><UserPlus size={18} /> Register</>)}
          </button>
        </form>
